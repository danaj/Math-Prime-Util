...
my @divt = (
  ...
  [17, "340282366920938463463374607431768211456", 0],
  [0,  "340282366920938463463374607431768211456", 1],
  # Our additional single divisor tests
  # Check UV n with negative d larger than IV
  ["3000000001","-3000000001", 1],
...
plan tests => 1 + 9*5 + scalar(@divt) + 3   # is_divisible
...
my $ntests = $extra && $use64 && $usexs ? 10000
           ...
           : $extra                     ?   250
           :                                 25;
my @r32 = map { irand() } 1 .. $ntests;
my @r64 = map { irand64() } 1 .. $ntests;
...
is(is_divisible(6,3,undef),1,"is_divisible stops after finding a divisor");
